...
export const Game = () => {
  ...
  React.useEffect(() => {
    if ((prevStageRef.current === 'setup' || prevStageRef.current === 'ended') && gameStage === 'playing') {
  ...
  const renderOpponentsTable = () => {
    ...
    const myIndex = isMultiplayer ? players.findIndex(p => p.id === myPlayerId) : -1;
        ...
        <div className="absolute top-1.5 left-0 right-0 w-full flex items-center justify-center gap-3 z-20 select-none">
          {opponents.map((player) => {
            ...
            return (
              ...
              >
                {/* Avatar */}
                <div className={`h-4 w-4 rounded-full bg-gradient-to-br ${player.avatarGradient} flex items-center justify-center text-[7px] font-black text-white`}>
                  {player.name.substring(0, 2).toUpperCase()}
                </div>

                {/* Name */}
                <span className="text-[8px] font-bold text-white truncate max-w-[50px]">{player.name}</span>
                ...
                <div className="flex items-center gap-0.5 bg-black/40 rounded px-1 py-0.25">
                  ...
                  <span className="text-[7px] font-black text-red-400">{player.hand.length}</span>
                </div>
    ...
    return players.map((player, i) => {
      ...
      const relativeIndex = myIndex !== -1 ? (i - myIndex + totalPlayers) % totalPlayers : i;
            ...
            {player.hand.map((card, cIndex) => (
              <div
                ...
                style={{
                  ...
                  marginLeft: cIndex > 0 ? (isMobile ? '-8px' : '-10px') : '0px'
                }}
              ></div>
            ))}
          </div>
    ...
    <div className={`min-h-screen relative flex flex-col ${gameStage === 'playing' || gameStage === 'ended' || (gameStage === 'setup' && isLandscapeMobile) ? 'overflow-hidden' : 'overflow-y-auto'}`}>
          ...
          <div className={`glass-panel w-full border border-white/10 shadow-2xl flex flex-col relative overflow-hidden ${
            ...
            {isMultiplayer ? (
              <div className={`flex items-center justify-between rounded-2xl bg-white/5 border border-white/10 shadow-inner ${
                isLandscapeMobile ? 'flex-row gap-2 p-2 rounded-xl' : 'flex-col sm:flex-row gap-4 p-4.5'
              }`}>
                <div className="flex flex-col gap-0.5">
                  <span className={`text-slate-400 font-bold uppercase tracking-wider ${isLandscapeMobile ? 'text-[8px]' : 'text-xs'}`}>Room Code</span>
                  <div className="flex items-center gap-2">
                    <span className={`font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-500 tracking-widest ${isLandscapeMobile ? 'text-base' : 'text-2xl'}`}>{roomCode}</span>
                    <button
                      type="button"
                      onClick={() => {
                ...
                <div className={`text-right text-slate-500 font-medium ${isLandscapeMobile ? 'text-[8px]' : 'text-xs'}`}>
                  {isHost ? (
                    <span className="text-emerald-400 font-bold">You are the HOST.</span>
                  ) : (
            ...
            <div className={`flex border-t border-white/5 ${
              ...
              >
                ...
                <span>{isMultiplayer && !isHost ? 'Waiting...' : 'Start Match'}</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {(gameStage === 'playing' || gameStage === 'ended') && (
        <div className={`flex-1 flex flex-col justify-between relative ${
          ...
          <div className={`w-full flex items-center justify-between z-30 gap-2 ${isLandscapeMobile ? 'mb-0' : 'mb-1 sm:mb-2'}`}>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <button
                ...
                title="Move History"
              >
                <History className={`${isLandscapeMobile ? 'h-3.5 w-3.5' : 'h-4 w-4 sm:h-5 sm:w-5'}`} />
              </button>

              {/* Quit / Leave Game button */}
              <button
                onClick={isMultiplayer && !isHost ? handleLeaveGame : quitToLobby}
                className={`glass-panel border-white/5 text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-xl sm:rounded-2xl font-black uppercase tracking-wider transition-all ${
                  isLandscapeMobile ? 'px-2 py-1 text-[8px]' : 'px-2.5 py-1.5 sm:px-4 sm:py-3 text-[10px] sm:text-xs'
                }`}
              >
          ...
          <div className={`flex-1 w-full mx-auto relative glass-panel-light border border-white/5 shadow-2xl flex items-center justify-center overflow-hidden ${
            isLandscapeMobile 
          ...
          <div className={`w-full flex flex-col items-center z-30 ${isLandscapeMobile ? 'gap-0' : 'gap-1 sm:gap-4'}`}>
            {/* UNO declarer alarms */}
            <UNOButton
              isCurrentTurn={isMyTurn}
              handSize={isMultiplayer ? (players.find(p => p.id === myPlayerId)?.hand?.length || 0) : (players[currentTurn]?.hand?.length || 0)}
              hasDeclaredUno={isMultiplayer ? (players.find(p => p.id === myPlayerId)?.declaredUno || false) : (players[currentTurn]?.declaredUno || false)}
              onDeclare={declareUno}
            />

            {/* Current Active Player Cards hand */}
            <PlayerHand
              hand={isMultiplayer ? (players.find(p => p.id === myPlayerId)?.hand || []) : (players[currentTurn]?.hand || [])}
              onPlayCard={playCard}
              isCurrentTurn={isMyTurn}
              topCard={discardPile[discardPile.length - 1]}
              currentColor={currentColor}
              playerName={isMultiplayer ? (players.find(p => p.id === myPlayerId)?.name || '') : (players[currentTurn]?.name || '')}
              onRename={isMultiplayer 
                ? (newName) => updateLobbyPlayerName(myPlayerId, newName)
                : (newName) => updateLobbyPlayerName(players[currentTurn]?.id, newName)
              }
          ...
          `}>
            ...
            <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar flex flex-col gap-2.5">
              {moveHistory.length === 0 ? (
                ...
                moveHistory.map((log) => (
                  <div key={log.id} className="p-2.5 rounded-xl bg-white/2 border border-white/5 flex flex-col gap-1 shadow-inner">
